...
import { FileTextOutlined, ShareAltOutlined, BarChartOutlined, CheckOutlined, LoadingOutlined, FileMarkdownOutlined, SafetyCertificateOutlined } from '@ant-design/icons'
...
type TabKey = 'results' | 'graph' | 'charts' | 'report' | 'review'
...
export default function ResearchDetail({ data, steps = [], onStepClick, onClose }: ResearchDetailProps) {
  ...
  const tabs: { key: TabKey; label: string; icon: React.ReactNode; count?: number }[] = [
    ...
    },
    {
      ...
      count: data?.qualityReview?.issues_count,
    },
  ]
      ...
      <div className={styles.content}>
        ...
        {activeTab === 'review' && (
          <div style={{ padding: 24 }}>
            {!data?.qualityReview ? (
              <div style={{ color: '#999', textAlign: 'center', padding: 48 }}>暂无质量审核结果</div>
            ) : (
              <>
                {data.qualityReview.is_current === false && (
                  <div style={{ padding: 12, marginBottom: 16, background: '#fff7e6', color: '#ad6800', borderRadius: 6 }}>
                    这是修订前报告的历史审核结果，不代表当前报告质量。
                  </div>
                )}
                <div style={{ display: 'flex', gap: 32, alignItems: 'center', marginBottom: 20 }}>
                  <div style={{ fontSize: 30, fontWeight: 700 }}>{data.qualityReview.quality_score}/10</div>
                  <div>
                    <div style={{ fontWeight: 600 }}>{data.qualityReview.verdict === 'pass' ? '审核通过' : '仍需改进'}</div>
                    <div style={{ color: '#666', marginTop: 4 }}>
                      共 {data.qualityReview.issues_count ?? data.qualityReview.issues?.length ?? 0} 项问题，
                      严重 {data.qualityReview.critical_issues ?? 0} 项，主要 {data.qualityReview.major_issues ?? 0} 项
                    </div>
                  </div>
                </div>
                {data.qualityReview.summary && <p>{data.qualityReview.summary}</p>}
                <div>
                  {(data.qualityReview.issues || []).map((issue, index) => (
                    <div key={issue.id || index} style={{ padding: '14px 0', borderTop: '1px solid #eee' }}>
                      <div style={{ fontWeight: 600 }}>[{issue.severity || 'minor'}] {issue.description || issue.issue_type}</div>
                      {issue.suggestion && <div style={{ color: '#666', marginTop: 6 }}>建议：{issue.suggestion}</div>}
                    </div>
                  ))}
                </div>
              </>
