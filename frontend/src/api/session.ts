...
/**
 * 从最近的研究检查点继续执行。响应为 SSE 流；调用方读取结束后应重新加载检查点。
 */
export function resumeResearch(sessionId: string, options?: AxiosRequestConfig) {
  return request.post<ReadableStream>(`/research/resume/${sessionId}`, {}, {
    headers: {
      Accept: 'text/event-stream',
    },
    responseType: 'stream',
