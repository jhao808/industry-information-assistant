...
class AgentsConfig:
    ...
    architect: AgentModelConfig = field(default_factory=lambda: AgentModelConfig(
        model=os.getenv("LLM_MAIN_MODEL", "deepseek-v4-pro-0813"),
        temperature=0.7,
        max_tokens=4000
    ))
    ...
    scout: AgentModelConfig = field(default_factory=lambda: AgentModelConfig(
        model=os.getenv("LLM_FAST_MODEL", "qwen3.8-flash"),  # 搜索阶段用快速模型
        temperature=0.5,
        max_tokens=4000
    ))
    ...
    critic: AgentModelConfig = field(default_factory=lambda: AgentModelConfig(
        model=os.getenv("LLM_REVIEW_MODEL", os.getenv("LLM_MAIN_MODEL", "deepseek-v4-pro-0813")),
        temperature=0.2,
        max_tokens=16000
    ))
...
class LLMConfig:
    ...
    # 默认模型（用于未单独配置的场景）
    default_model: str = field(default_factory=lambda: os.getenv("LLM_MAIN_MODEL", "deepseek-v4-pro-0813"))
    ...
    def to_dict(self) -> Dict[str, Any]:
        """转换为字典格式"""
        return {
            "api_key": "[configured]" if self.api_key else "",
            "base_url": self.base_url,
            "search_api_key": "[configured]" if self.search_api_key else "",
