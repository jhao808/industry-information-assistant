...
class DeepResearchGraph:
    ...
    def __init__(
        ...
        # 搜索整理模型额度耗尽时，复用主模型继续处理已取得的网页结果。
        self.scout.fallback_model = config.agents.data_analyst.model
        ...
        if resume and session_id:
            ...
            if state:
                if state.get("phase") == ResearchPhase.COMPLETED.value:
                    yield {"type": "error", "content": "研究已经完成，不能重复恢复"}
                    return
                yield {
    ...
    async def _run_simplified(self, state: ResearchState) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        # 阶段完成标记随检查点持久化；旧检查点由已保存的阶段 UI 状态迁移。
        completed_phases = set(state.get("completed_phases") or [])
        if not completed_phases and state.get("outline"):
            completed_phases.add("planning")
        state["completed_phases"] = list(completed_phases)

        if state.get("_resuming") and self.checkpoint_service and state.get("session_id"):
            previous = self.checkpoint_service.load_full_checkpoint(state["session_id"])
            if previous and previous.get("ui_state_json"):
                old_ui = previous["ui_state_json"]
                ui_state.update({key: old_ui[key] for key in ui_state if key in old_ui})
                for step in old_ui.get("research_steps", []):
                    if step.get("status") == "completed":
                        completed_phases.add(step.get("type"))
                state["completed_phases"] = list(completed_phases)
        # 旧版检查点可能只搜索前三章，却把整个搜索阶段标为完成。
        if any(section.get("status") == "pending" for section in state.get("outline", [])):
            completed_phases.discard("researching")
            state["completed_phases"] = list(completed_phases)
        ...
        def update_ui_state():
            ...
            if state.get("review_result"):
                ui_state["quality_review"] = state["review_result"]

            drafts = state.get("draft_sections", {})
            if drafts:
                ui_state["sections"] = [
                    {"id": section.get("id"), "title": section.get("title", ""),
                     "content": drafts[section["id"]], "wordCount": len(drafts[section["id"]])}
                    for section in state.get("outline", [])
                    if section.get("id") in drafts
                ]
        ...
        async def save_checkpoint_async(step_info: dict = None):
            ...
            if step_info:
                ...
                if step_info.get("status") == "completed":
                    completed_phases.add(step_info["type"])
                    state["completed_phases"] = list(completed_phases)
        ...
        try:
            ...
            if await check_cancelled():
                yield {"type": "research_cancelled", "message": "研究已取消"}
                return
            if "researching" not in completed_phases:
                ...
                cp_event = await save_checkpoint_async({
                    ...
                    "stats": {"facts": len(state.get("facts", [])), "sources": len(state.get("references", []))}
                })
                if cp_event:
                    yield cp_event
            ...
            if await check_cancelled():
                yield {"type": "research_cancelled", "message": "研究已取消"}
                return
            if "analyzing" not in completed_phases:
            ...
            if await check_cancelled():
                yield {"type": "research_cancelled", "message": "研究已取消"}
                return
            if "writing" not in completed_phases:
            ...
            while state["iteration"] < state["max_iterations"]:
                ...
                async for msg in run_agent_with_streaming(self.critic):
                    yield msg
                state["messages"] = []
                # 审核 Agent 成功返回即表示它检查了调用时的报告版本；后续写作会再次置为失效。
                state["review_is_current"] = True

                cp_event = await save_checkpoint_async({
                    "type": "reviewing", "status": "completed",
                    "stats": {"quality_score": state.get("quality_score", 0),
                              "issues": len(state.get("critic_feedback", []))}
                })
                if cp_event:
                    yield cp_event
            ...
            # 报告在审核后经过了补充搜索或修订，必须对当前最终版本重新审核。
            if state.get("final_report") and not state.get("review_is_current", False):
                if await check_cancelled():
                    yield {"type": "research_cancelled", "message": "研究已取消"}
                    return
                yield {"type": "phase", "phase": "reviewing", "content": "正在复审最终报告..."}
                state["phase"] = ResearchPhase.REVIEWING.value
                state["_final_review_only"] = True
                try:
                    async for msg in run_agent_with_streaming(self.critic):
                        yield msg
                finally:
                    state.pop("_final_review_only", None)
                state["messages"] = []
                cp_event = await save_checkpoint_async({
                    "type": "reviewing", "status": "completed",
                    "stats": {"quality_score": state.get("quality_score", 0),
            ...
            # 修订后的最终报告也需要保存，不能只更新完成标记。
            final_checkpoint = await save_checkpoint_async()
            if final_checkpoint:
            ...
            yield {
                ...
                "quality_review": state.get("review_result"),
