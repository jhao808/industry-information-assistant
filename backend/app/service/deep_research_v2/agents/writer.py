...
from .base import BaseAgent, IncompleteLLMResponseError
from ..state import ResearchState, ResearchPhase
from ..evidence import writing_instruction, guard_draft, lacks_evidence, DRAFT_NOTICE
...
```"""
    ...
    def __init__(self, llm_api_key: str, llm_base_url: str, model: str = "qwen3.8-flash"):
        super().__init__(
            name="LeadWriter",
            role="首席笔杆",
    ...
    async def process(self, state: ResearchState) -> ResearchState:
        """处理入口"""
        if lacks_evidence(state):
            state["references"] = []
            self.add_message(state, "thought", {"agent": self.name, "content": DRAFT_NOTICE})
    ...
    async def _write_section(self, state: ResearchState, section: Dict) -> None:
        ...
        response = await self.call_llm(
            ...
            user_prompt=writing_instruction(state) + prompt,
            json_mode=True,
            temperature=0.4,
        ...
        if result and result.get("content"):
            section_content = guard_draft(state, result["content"])
            state["draft_sections"][section_id] = section_content
            section["status"] = "drafted"

            # 收集引用
            for citation in (result.get("citations", []) if not lacks_evidence(state) else []):
    ...
    async def _synthesize_report(self, state: ResearchState) -> None:
        ...
        result = None
        for max_tokens in (32768, 65536):
            self.logger.info("[LeadWriter] 调用 LLM 整合报告，max_tokens=%s...", max_tokens)
            try:
                response = await self.call_llm(
                ...
                if result and result.get("full_report"):
                    break
                self.logger.warning(
                    "[LeadWriter] 报告整合未返回有效 full_report，%s",
                    "将扩大输出额度重试" if max_tokens == 32768 else "将使用章节草稿兜底",
                )
            except IncompleteLLMResponseError:
                self.logger.warning(
                    "[LeadWriter] 报告整合输出被截断，%s",
                    "将扩大输出额度重试" if max_tokens == 32768 else "将使用章节草稿兜底",
                )
        ...
        if result and result.get("full_report"):
            ...
            # 更新参考文献
            for ref in (result.get("references", []) if not lacks_evidence(state) else []):
                if ref not in state["references"]:
                    state["references"].append(ref)
        else:
            # 两次结构化整合均失败时，保留已有研究成果并生成可审核的报告。
            self.logger.warning("[LeadWriter] ⚠️ 模型整合失败，使用章节草稿组装最终报告")
            state["final_report"] = self._build_report_from_sections(state)
            state["report_generation_mode"] = "section_fallback"
            self.logger.info(f"[LeadWriter] 使用备选报告，长度: {len(state['final_report'])}")

        state["final_report"] = guard_draft(state, state["final_report"])
        self._mark_report_changed(state)
        if lacks_evidence(state):
            executive_summary = guard_draft(state, executive_summary) if executive_summary else ""
            conclusions = [guard_draft(state, item) for item in conclusions]
    ...
    @staticmethod
    def _build_report_from_sections(state: ResearchState) -> str:
        """按大纲顺序把已保存的章节草稿组合为一份完整报告。"""
        parts = [f"# {state['query']} 研究报告"]
    ...
    @staticmethod
    def _mark_report_changed(state: ResearchState) -> None:
        """正式报告变化后，使旧审核结论失效。"""
        state["report_version"] = int(state.get("report_version", 0)) + 1
        state["review_is_current"] = False
    ...
    async def _revise_report(self, state: ResearchState) -> ResearchState:
        ...
        if result and result.get("revised_content"):
            state["final_report"] = guard_draft(state, result["revised_content"])
            self._mark_report_changed(state)
