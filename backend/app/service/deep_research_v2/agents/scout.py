...
```"""
    ...
    def _normalize_search_queries(raw_queries: Any) -> List[str]:
        ...
        for item in raw_queries:
            if isinstance(item, str):
                ...
                query = next((
                    item.get(key) for key in ("query", "search_query", "keyword", "content", "value")
    ...
    async def process(self, state: ResearchState) -> ResearchState:
        ...
        # 恢复时从已有事实重建去重索引，避免重复追加同一事实。
        self.fact_fingerprints = {
            self._compute_fact_fingerprint(f.get("content", "")): f.get("source_url", "")
            for f in state.get("facts", []) if f.get("content")
        }
        ...
        # 并行研究多个章节
        for offset in range(0, len(pending_sections), 3):
            await asyncio.gather(*(self._research_section(state, section)
                                   for section in pending_sections[offset:offset + 3]))
...
```"""
    ...
    async def _research_section(self, state: ResearchState, section: Dict) -> None:
        ...
        if analysis:
            ...
            # 递归搜索：信源追溯查询（优先级最高）
            source_tracing = self._normalize_search_queries(
            ...
            # 递归搜索：追踪发现的新线索
            follow_up = self._normalize_search_queries(
                analysis.get("follow_up_queries", [])
            )
    ...
    async def _execute_deep_search(
        ...
        for query in queries:
            ...
            if depth < max_depth:
                further_tracing = self._normalize_search_queries(
                    analysis.get("further_tracing_queries", [])
                )
                if further_tracing:
                    self.add_message(state, "thought", {
                        "agent": self.name,
        ...
        try:
            async with self._milvus_lock:
                if self.milvus_service is None:
                    self.milvus_service = await asyncio.to_thread(MilvusService)
