...
```"""
    ...
    async def process(self, state: ResearchState) -> ResearchState:
        ...
        self._validate_review_result(review_result)

        if review_result:
            report_hash = hashlib.sha256(state.get("final_report", "").encode("utf-8")).hexdigest()
            previous_review = state.get("review_result")
            if previous_review:
                state.setdefault("review_history", []).append(previous_review)
            ...
            state["critic_feedback"] = current_issues
            ...
            state["review_result"] = {
                ...
                "critical_issues": len([i for i in current_issues if i.get("severity") == "critical"]),
                "major_issues": len([i for i in current_issues if i.get("severity") == "major"]),
                "missing_aspects": review_result.get("missing_aspects", []),
            }
            state["review_is_current"] = True
            state["review_status"] = "passed" if assessment.get("verdict") == "pass" else "needs_revision"
    ...
    def _validate_review_result(result: Dict[str, Any]) -> None:
        """没有有效审核结论时停止流水线，避免误报完成。"""
        assessment = result.get("overall_assessment") if isinstance(result, dict) else None
        ...
        if isinstance(score, bool) or not isinstance(score, (int, float)) or not 1 <= score <= 10:
            raise ValueError("质量审核返回了无效的 quality_score")
        if verdict not in {"pass", "needs_revision", "major_issues"}:
            raise ValueError("质量审核返回了无效的 verdict")
        if not isinstance(result.get("issues"), list):
            raise ValueError("质量审核未返回 issues 列表")
        if verdict == "pass" and score < 7:
            raise ValueError("质量审核的通过结论与评分冲突")
    ...
    async def _review_content(self, state: ResearchState) -> Dict[str, Any]:
        ...
        # 始终优先审核用户最终看到的正式报告，避免给旧章节草稿打分。
        draft_content = state.get("final_report", "")
        ...
        # DeepSeek V4 的 max_tokens 同时计入思维链和正文；保留推理，给 JSON 足够的剩余额度。
        for max_tokens in (32768, 65536):
            try:
                ...
                if max_tokens == 65536:
                    raise
                self.logger.warning("审核响应达到长度上限，增加输出额度后重试")
                continue

            result = self.parse_json_response(response)
            self.logger.info("[CriticMaster] JSON 解析结果: %s", bool(result))
            if result:
                self._validate_review_result(result)
                return result
            if max_tokens == 65536:
                raise ValueError("质量审核返回的 JSON 无法解析")
            self.logger.warning("审核 JSON 无法解析，增加输出额度后重试")

        raise ValueError("质量审核没有得到有效结果")
