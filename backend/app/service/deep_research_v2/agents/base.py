...
class IncompleteLLMResponseError(ValueError):
    """模型因输出额度耗尽而截断结构化结果。"""
...
class BaseAgent(ABC):
    ...
    async def call_llm(
        ...
        try:
            ...
            self.logger.info(
                "LLM call completed in %sms, model=%s, finish_reason=%s, "
                "response_length=%s, reasoning_length=%s",
                duration, self.model, choice.finish_reason, len(content),
                len(choice.message.reasoning_content or "") if hasattr(choice.message, "reasoning_content") else 0,
            )

            if json_mode and choice.finish_reason == "length":
                raise IncompleteLLMResponseError("结构化输出达到长度上限，响应可能不完整")
            ...
            quota_error = "insufficient_quota" in error_text or "free quota exhausted" in error_text
            if quota_error and self.fallback_model and self.fallback_model != self.model:
                failed_model = self.model
                self.model = self.fallback_model
                self.logger.warning(
                    "Model %s quota exhausted; switching %s to fallback model %s",
                    failed_model, self.name, self.model,
                )
