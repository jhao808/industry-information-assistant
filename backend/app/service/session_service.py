...
class SessionService:
    ...
    def create_session(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        Returns:
            包含会话信息的字典
        """
        # Web 端的会话先在 PostgreSQL 中创建。普通问答必须沿用
        # 同一个 ID 建立 Redis 历史，否则每轮都会变成新对话。
    ...
    def add_message(
        ...
        # Redis sorted-set 的 score 必须保留亚秒精度。批量恢复历史记录时，
        # 若全部使用 int(time.time())，同一秒内的消息会退化为按随机 UUID
        # 排序，从而破坏真实对话顺序。
        ...
        # 添加到会话的消息列表（按时间排序）
        self.redis_client.zadd(f"session:{session_id}:messages", {message_id: float(timestamp)})
